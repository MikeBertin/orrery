...
# id (NAIF) · name · colour · window · step. Windows are generous; Horizons
# clamps to each craft's available SPK coverage and we keep whatever returns.
CRAFT = [
...
def main():
    ...
    out = []
    for cmd, name, color, start, stop, step in CRAFT:
        ...
        if not t:
            ...
            continue
        out.append({"name": name, "color": color, "t": t, "p": p})
